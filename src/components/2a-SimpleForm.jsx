...
function SimpleForm() {
  ...
  return (
    <div>
      ...
      <br />
      <input id="drink" type="text" value={drink} onChange={(event)=>{setDrink(event.target.value)}}/>
      <br /><br />

      {/* 3. TODO: Add value and onChange props to make this a controlled select */}
      <label htmlFor="milk">Preferred milk</label>
      <br />
      <select id="milk" value="milk" onChange={(event)=>{setMilk(event.target.value)}}>
      ...
      <div style={{ background: '#f5f0e8', padding: '0.75rem' }}>
        ...
        <p>Milk: {milk} </p>
        <p>Notes: {notes || "No special instructions provided"}</p>
      </div>
    </div>
    //null is a falsy value is zero
